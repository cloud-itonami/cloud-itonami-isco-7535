(ns tannerycoord.actor
  "TanneryCoordActor — the ISCO-08 7535 pelt dressers, tanners and
  fellmongers tannery scheduling/logistics coordination actor as a
  `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one tannery-coordination operation request
  ...
  The unconditional invariant: the Tannery Scheduling Coordination
  Advisor can never directly commit an operating record the
  TanneryCoordGovernor refuses, and can never make this actor finalize
  a tanning-execution decision (e.g. deciding to proceed with a
  specific pelt-tanning run) or a chemical-safety-clearance decision
  (e.g. declaring a tanned batch safe for handling or shipment), and
  can never override a shop safety officer's judgment — every
  ...
  actor coordinates TANNERY SCHEDULING/LOGISTICS ONLY — it never
...
(defn build-graph
  "Build a compiled TanneryCoordActor graph. `store` implements
  `tannerycoord.store/Store`. `advisor` implements
  `tannerycoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:tanner-id (:tanner-id request)
                                    :op (:op proposal)

(ns tannerycoord.actor-test
  (:require [clojure.test :refer [deftest is testing]]
            [tannerycoord.actor :as actor]
...
(deftest commits-a-registered-work-log
  (let [st (fresh-store)
        ...
        request {:tanner-id "tanner-1" :op :log-work-record :stake :low
                  :facility-id "F-1" :task "batch progress log"}
        result (actor/run-request! graph request {} "thread-1")]
    (is (= :done (:status result)))
    (is (some? (get-in result [:state :record])))
    (is (= 1 (count (store/records-of st "tanner-1"))))))
...
(deftest holds-an-unregistered-facility-proposal
  (let [st (fresh-store)
    ...
    (is (empty? (store/records-of st "tanner-1")))))

(deftest interrupts-then-approves-safety-concern-on-human-approval
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        request {:tanner-id "tanner-1" :op :flag-safety-concern :stake :low
...
(deftest holds-a-scope-excluded-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would finalize a tanning-execution decision, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:tanner-id "tanner-1" :op :finalize-tanning-decision :stake :low
                    :facility-id "F-1" :task "tanning decision"}
...
(deftest holds-a-chemical-safety-clearance-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would finalize a chemical-safety-clearance decision, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:tanner-id "tanner-1" :op :declare-batch-safe-for-handling :stake :low
...
(deftest holds-an-override-shop-safety-officer-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would override a shop safety officer's judgment, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:tanner-id "tanner-1" :op :override-shop-safety-officer-judgment :stake :low
